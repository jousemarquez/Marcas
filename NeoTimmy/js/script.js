...
function start() {
  ...
  gsap.timeline()
    ...
    .fromTo(".dot", { scale: 0.5, rotate: 0, opacity: 0, svgOrigin: "50 50" }, {
      scale: 1,
      opacity: 1,
      stagger: { grid: [10, 10], from: "center", amount: 0.4 },
      ease: "back.out(3)",
    }, 1)
    .fromTo("#bot, .stars use", { opacity: 0, scale: 1 }, {
      opacity: 1,
      stagger: 0.05,
      ease: "expo",
    }, 2)
    ...
    .add(removeDot)
    .fromTo("#timer", { transformOrigin: "50% 100%", rotate: 360 }, {
      rotate: 0,
      ease: "none",
      duration: 15 / (level / 5),
      onComplete: () => end(true),
  ...
  for (let i = 1; i <= 3; i++) {
    ...
    if (botPos.x == starPos.x && botPos.y == starPos.y) {
      ...
      gsap.timeline()
        ...
        .fromTo(".mouth", { attr: { d: "M3,6.8L4,6.8L6,6.8L7,6.8" } }, {
          attr: { d: "M3,6L4,7L6,7L7,6" },
          yoyo: true,
          repeat: 1,
          ease: "expo.inOut",
          repeatDelay: 0.2,
        ...
        .to(".markers use", {
          attr: { fill: (i) => (i + 1 <= nStars) ? "#fff" : "#000" },
          onComplete: end,
        }, 0);
    }
  }
...
function end(gameOver = false) {
  if (nStars == 3 || gameOver) {
    ...
    gsap.timeline()
      ...
      .to(".dot", {
        rotate: (level % 2 == 0) ? -45 : 45,
        scale: 0.6,
        opacity: 0,
        stagger: { grid: [10, 10], from: "center", amount: 0.6 },
        ease: "back.inOut",
      }, "-=0.5")
      .to(".markers use", {
        attr: { fill: "#000" },
      ...
      .fromTo(".txt1", {
        ...
        opacity: 0,
        y: 0,
        textContent: (gameOver) ? "GAME OVER" : "LEVEL " + level,
      }, { scale: 1, opacity: 1, ease: "back.out(3)" })
      .set("#bot", { x: 0, y: 0 })
      .add(() => {
        if (!gameOver) start();
        else {
          let best = (window.localStorage.best)
            ? Number(window.localStorage.best)
            : 0;
